...
#Random Forest Unemployment UK
RF_Unemployment_UK<- train(
  Unemployment ~ ., 
  data = train_UK, 
  method = "rf",
...
#Random Forest Inflation UK
RF_Inflation_UK<- train(
  Inflation ~ ., 
  data = train_UK, 
  method = "rf",
  tuneGrid = expand.grid(mtry =3:9),
...
#Random FOrest Unemployment US
RF_Unemployment_US<- train(
  Unemployment ~ ., 
  data = train_US, 
  method = "rf",
  tuneGrid = expand.grid(mtry =3:9),
...
#Random Forest Inflation US
RF_Inflation_US<- train(
  Inflation ~ ., 
  data = train_US, 
  method = "rf",
  tuneGrid = expand.grid(mtry =3:9),
...
#Funcions per calcular RMSE, Bias^2

rmse <- function(fitted, actual){
  sqrt(mean((fitted - actual)^2))
}
bias2 <-function(fitted,actual){
  mean((fitted - actual)^2)
} 
...
Stats_UK(train_UK$Unemployment,test_UK$Unemployment,RF_Unemployment_UK)
Stats_UK(train_UK$Inflation,test_UK$Inflation,RF_Inflation_UK)
Stats_UK(train_UK$GDP,test_UK$GDP,RF_GDP_UK)
...
Stats_US(train_US$Unemployment,test_US$Unemployment,RF_Unemployment_US)
Stats_US(train_US$Inflation,test_US$Inflation,RF_Inflation_US)
Stats_US(train_US$GDP,test_US$GDP,RF_GDP_US)
...
Shapley(RF_Unemployment_UK,"Unemployment","UK")
Shapley(RF_Inflation_UK,"Inflation","UK")
Shapley(RF_GDP_UK,"GDP","UK")
Shapley(RF_Unemployment_US,"Unemployment","US")
Shapley(RF_Inflation_US,"Inflation","US")
Shapley(RF_GDP_US,"GDP","US")
